...
function removeFirstOccurrences(str, value) {
  ...
  if (idx < 0) return str;
  return str.substring(0, idx) + str.substring(idx + value.length);
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < modifiedStr.length; i += 1) {
...
function isPalindrome(str) {
  ...
  const reversedStr = reverseString(modifiedStr);
  return modifiedStr.toLowerCase() === reversedStr.toLowerCase();
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < wordsArr.length; i += 1) {
    reversedWordsArr.push(reverseString(wordsArr[i]));
  }
...
function encodeToRot13(str) {
  ...
  function getLetter(letters, i) {
    const idx = letters.indexOf(str[i]);
    if (idx > 12) {
      return letters[Math.abs(idx + 13 - 26)];
    }
    return letters[idx + 13];
  }

  for (let i = 0; i < str.length; i += 1) {
    if (lowercaseLetters.includes(str[i])) {
      modifiedStr += getLetter(lowercaseLetters, i);
    } else if (uppercaseLetters.includes(str[i])) {
      modifiedStr += getLetter(uppercaseLetters, i);
    } else {
      modifiedStr += str[i];
    }
  }
